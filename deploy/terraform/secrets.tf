...
resource "aws_secretsmanager_secret_version" "secrets" {
  ...
  secret_string = jsonencode({
    ...
    COGNITO_JWKS_URL            = "https://${aws_cognito_user_pool.this.endpoint}/.well-known/jwks.json"
    MS_TOKEN_SECRET             = base64encode("${aws_cognito_user_pool.this.id}:${aws_cognito_user_pool_client.this.id}")
  })
